...
use super::{
    get_active_sessions, get_bytes_sent_delta, get_per_protocol_active,
    get_per_protocol_egress_delta,
};

const NAMESPACE: &str = "vpnVPN";

pub async fn start_publisher_task() {
    if env::var("DISABLE_CLOUDWATCH_METRICS")
        .map(|v| v == "1")
        .unwrap_or(false)
    {
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            let protocols: Vec<&str> = protos_env
                .split(',')
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .collect();
