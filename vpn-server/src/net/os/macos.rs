...
impl NetworkManager for MacNetworkManager {
    fn check_dependencies(&self) -> Result<()> {
        ...
        if Command::new("wg").arg("version").output().is_err() {
            return Err(anyhow!("'wg' command not found (brew install wireguard-tools)"));
        }
        // Check for 'wireguard-go'
        if Command::new("wireguard-go").arg("--version").output().is_err() {
            return Err(anyhow!("'wireguard-go' not found (brew install wireguard-go)"));
        }
        Ok(())
    }

    fn configure_wireguard(&self, iface: &str, ip_cidr: &str, _port: u16, conf_path: &str) -> Result<()> {
        ...
        if !iface_exists {
            ...
            let _ = Command::new("wireguard-go").arg("utun").env("WG_TUN_NAME_FILE", format!("/tmp/{}.name", iface)).output();
