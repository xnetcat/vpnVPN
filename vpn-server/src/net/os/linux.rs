...
impl NetworkManager for LinuxNetworkManager {
    ...
    fn configure_wireguard(&self, iface: &str, ip_cidr: &str, _port: u16, conf_path: &str) -> Result<()> {
        ...
        if !iface_exists {
             ...
             if let Ok(_) = Command::new("wireguard-go").arg(iface).spawn() {
                 ...
                 let _ = Command::new("ip").args(["link", "add", iface, "type", "wireguard"]).output();
             }
        }

        // 2. Assign IP
        let _ = Command::new("ip").args(["address", "add", ip_cidr, "dev", iface]).output();
    ...
    fn teardown_interface(&self, iface: &str) -> Result<()> {
        let _ = Command::new("ip").args(["link", "set", "down", "dev", iface]).output();
