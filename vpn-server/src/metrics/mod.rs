...
pub static BYTES_TRANSFERS_PROTO: Lazy<IntCounterVec> = Lazy::new(|| {
    let vec = IntCounterVec::new(
        Opts::new("vpn_transfer_bytes_total_by_protocol", "Transferred bytes by protocol and direction"),
        &["protocol", "direction"],
    )
...
static LAST_EGRESS_BY_PROTOCOL: Lazy<Mutex<HashMap<String, u64>>> = Lazy::new(|| Mutex::new(HashMap::new()));
pub static SESSIONS_ACTIVE_PROTO: Lazy<IntGaugeVec> = Lazy::new(|| {
    let vec = IntGaugeVec::new(
        Opts::new("vpn_active_sessions_by_protocol", "Active VPN sessions per protocol"),
        &["protocol"],
    )
...
pub fn record_active_sessions_for(protocol: &str, total: usize) {
    SESSIONS_ACTIVE_PROTO.with_label_values(&[protocol]).set(total as i64);
}

pub fn record_transfer_for(protocol: &str, direction: &str, bytes: u64) {
    if bytes == 0 { return; }
    BYTES_TRANSFERS_PROTO.with_label_values(&[protocol, direction]).inc_by(bytes);
}
...
pub fn get_bytes_sent_delta() -> u64 { get_egress_delta() }
...
pub fn get_per_protocol_active(protocols: &[&str]) -> Vec<(String, u64)> {
    protocols
        ...
        .map(|p| (
            (*p).to_string(),
            SESSIONS_ACTIVE_PROTO.with_label_values(&[p]).get().try_into().unwrap_or_default(),
        ))
        .collect()
}
