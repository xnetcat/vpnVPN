...
Platforma będzie składała się z czterech głównych komponentów:
...
- **Control Plane (backend)** – serwerlessowa warstwa sterująca (AWS API Gateway + Lambda z obrazami Docker + PostgreSQL/Neon), która przechowuje stan systemu (użytkownicy, urządzenia/peers, serwery VPN, tokeny), udostępnia API do rejestracji węzłów, synchronizacji konfiguracji oraz obsługi webhooków płatności.
- **VPN Server (data plane)** – autonomiczny agent napisany w Rust, uruchamiany jako kontener Docker na instancjach EC2 z elastycznymi adresami IP, odpowiedzialny za zestawianie tuneli VPN (WireGuard / OpenVPN / IKEv2), cykliczną synchronizację listy dozwolonych peerów z Control Plane oraz raportowanie zanonimizowanych metryk.
- **Desktop Client** – aplikacja desktopowa Tauri z uprzywilejowanym daemonem Rust komunikującym się przez IPC (Unix socket), obsługująca WireGuard, OpenVPN i IKEv2 na macOS, Linux i Windows.
...
- **Dokumentacja podprojektów**:
  - Frontend: opis wykorzystania Next.js (App Router, TypeScript, TailwindCSS), Prisma + PostgreSQL, Auth.js/NextAuth, Stripe Checkout/Portal, konfiguracja środowiska i wdrożenia na Vercel.
  - VPN Server: opis gotowych endpointów administracyjnych (`/health`, `/metrics`, `/status`, `/pubkey`), sposobu nasłuchiwania na portach VPN (WireGuard UDP 51820, OpenVPN UDP 1194, IKEv2 UDP 500/4500), konfiguracji przez zmienne środowiskowe oraz integracji z metrykami (CloudWatch).
  - Desktop Client: architektura z podziałem na uprzywilejowany daemon i GUI Tauri, komunikacja IPC przez Unix socket, obsługa wielu protokołów VPN.
  - Infra/Pulumi: zarys infrastruktury jako kodu (Pulumi TS), wykorzystanie AWS (PostgreSQL/Neon, Lambda z obrazami Docker, API Gateway, EC2 z Elastic IP dla węzłów VPN, ECR, S3).
- **Wstępny model danych** – definicje obiektów typu `User`, `Device/Peer`, `Server`, `VpnToken` i sposób powiązania ich z subskrypcjami oraz konfiguracją sieciową (Prisma schema).
...
3. **Analiza wymagań i projekt systemu vpnVPN**
   ...
   | ---------------------------------- | ------------------------------------------------------------------------------- | ------------------------------------------------------------- | ----------------------------------------------- |
   | AWS (Lambda + API GW + PostgreSQL) | Dojrzałe usługi serverless, obrazy Docker na Lambda, Neon jako managed Postgres | Vendor lock‑in, złożony model cenowy                          | **Wybrana jako główna platforma Control Plane** |
...
4. **Implementacja platformy vpnVPN**
   ...
   - **Control Plane (AWS Serverless)**:
     - Definicja infrastruktury jako kodu (Pulumi – PostgreSQL/Neon, API Gateway, Lambda z obrazami Docker, ECR).
     - Implementacja kluczowych endpointów (`/server/register`, `/server/peers`, `/peers`, `/servers`, `/tokens`, webhooki Stripe).
   - **VPN Server (Rust)**:
